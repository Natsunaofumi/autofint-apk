...
def main(page: ft.Page):
    ...
    # Ukuran window hanya simulasi di PC, di HP akan full screen otomatis
    ...
    # --- DATABASE SETUP ---
    # Di Android, ini akan membuat file di folder data aplikasi
    ...
    state = {
        ...
        "edit_date": None,
        "total_cash": 0,
        "is_logged_in": False, # Status Login
        "user_pin": "1234"     # PIN Default
    }
    ...
    def save_file_result(e: ft.FilePickerResultEvent):
        if e.path:
            try:
                ...
                # Tulis file ke path yang dipilih user
                with open(e.path, 'w', newline='') as f:
                    writer = csv.writer(f)
                    writer.writerow(["ID", "Tanggal", "Tipe", "Kategori", "Deskripsi", "Jumlah", "Is Tabungan"]) # Header
                    writer.writerows(rows)
                    
                page.show_snack_bar(ft.SnackBar(ft.Text(f"Berhasil disimpan di: {e.path}"), bgcolor="green"))
            except Exception as ex:
                page.show_snack_bar(ft.SnackBar(ft.Text(f"Gagal menyimpan: {str(ex)}"), bgcolor="red"))

    file_picker = ft.FilePicker(on_result=save_file_result)
    page.overlay.append(file_picker)

    def export_csv(e):
        # Membuka dialog simpan file native Android/OS
        file_picker.save_file(file_name="Laporan_Keuangan.csv", allowed_extensions=["csv"])

    # --- UI COMPONENTS: LOGIN SCREEN (NEW) ---
    def view_login():
        input_pin = ft.TextField(
            ...
            input_filter=ft.InputFilter(regex_string=r"[0-9]"), # Pastikan hanya angka
            hint_text="PIN (Default: 1234)",
            on_submit=lambda e: check_pin(e) # Bisa tekan Enter
        )
        
        def check_pin(e):
            # Menggunakan strip() untuk menghapus spasi yang tidak sengaja
            if input_pin.value and input_pin.value.strip() == state["user_pin"]:
                state["is_logged_in"] = True
                input_pin.value = "" # Clear PIN
                navigate_to(0) # Masuk ke Dashboard
            else:
    ...
    # ListView diganti Column agar bisa scroll bareng halaman utama
    lv_dashboard = ft.Column(spacing=10)

    # Helper untuk membuat tombol stat yang bisa diklik
            ...
            content=ft.Column([
                ...
                ft.Container(
                    ...
                    content=ft.Column([
                        ...
                        # --- INTERAKTIF: SISA SALDO -> SIMULATOR ---
                        ft.Container(
                            content=ft.Column([
                ...
                ft.Container(
                    ...
                    content=ft.Column([
                        ...
                        txt_search # Menambahkan Search Bar
                    ])
                ),
                # Container padding untuk List Transaksi
                ft.Container(
                    content=lv_dashboard,
                    padding=20
                ),
                get_watermark() 
            ], spacing=0, scroll=ft.ScrollMode.AUTO), # Scroll utama halaman
            expand=True
        )

    # --- UI COMPONENTS: LAPORAN (FILTER + SIMULATOR) ---
    ...
    def open_simulator(e):
        ...
        # BottomSheet dengan warna adaptif
        bs = ft.BottomSheet(
            ft.Container(
            ...
            for i, row in enumerate(data_chart):
                ...
                chart_pie.sections.append(
                    ft.PieChartSection(
                        ...
                        # Badge untuk Nama Kategori (Di Luar)
                        badge=ft.Container(
